...
public class _11_Data_Extractor_Example {
    ...
    public static void numberExtractor(DataExtractorAssistant geminiDataExtractor) {
        // Extract Numbers
        String request = """
        ...
        int intNumber = geminiDataExtractor.extractInt(request);
        AiBeans.printResult(request, "Number = "+intNumber);
    }

    public static void DateTimeExtractor(DataExtractorAssistant geminiDataExtractor) {
        ...
        LocalDate date = geminiDataExtractor.extractDateFrom(request);
        sb.append("Date      = ").append(date).append("\n");
        LocalTime time = geminiDataExtractor.extractTimeFrom(request);
        sb.append("Time      = ").append(time).append("\n");
        LocalDateTime dateTime = geminiDataExtractor.extractDateTimeFrom(request);
        sb.append("DateTime = ").append(dateTime);

        AiBeans.printResult(request, sb.toString());
    }

    public static void pojoExtractor(DataExtractorAssistant geminiDataExtractor) {
        ...
        Person person = geminiDataExtractor.extractPersonFrom(request);
        AiBeans.printResult(request, person.toString());
    }

    public static void complexPojoExtractor(ChatLanguageModel geminiChatModel) {
        ChefAssistant chefAssistant = AiServices.create(ChefAssistant.class, geminiChatModel);
    ...
    public static void main(String[] args) {
        ...
        DataExtractorAssistant geminiDataExtractor = AiServices.create(DataExtractorAssistant.class, modelGemini);
        try {
            Std.println("Number Extractor =================================================");
            // Extract Numbers
            numberExtractor(geminiDataExtractor);
        } catch (Exception e) {
        ...
        try {
            ...
            // Extract Date and Time
            DateTimeExtractor(geminiDataExtractor);
        } catch (Exception e) {
            Std.println(ERROR_MESSAGE+e.getMessage());
        }
        try {
            ...
            // POJO Person Extractor
             pojoExtractor(geminiDataExtractor);
        } catch (Exception e) {
            Std.println(ERROR_MESSAGE+e.getMessage());
        }
        try {
            ...
            complexPojoExtractor(modelGemini);
        } catch (Exception e) {
            Std.println(ERROR_MESSAGE+e.getMessage());
        }
    }
