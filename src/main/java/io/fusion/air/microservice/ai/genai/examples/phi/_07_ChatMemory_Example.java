...
public class _07_ChatMemory_Example {
    ...
    public static void chatMemoryConversations(ChatLanguageModel phi3ChatModel) {
        Tokenizer tokenizer = new OpenAiTokenizer(AiConstants.GPT_4_TURBO);
        ChatMemory chatMemoryPhi3 = TokenWindowChatMemory.withMaxTokens(2000, tokenizer);
        ...
        SystemMessage systemMessage = SystemMessage.from(
                """
                        ...
                        """);
        chatMemoryPhi3.add(systemMessage);
        ...
        UserMessage userMessage1 = userMessage(
                """
                 ...
                 """);
        chatMemoryPhi3.add(userMessage1);
        Response<AiMessage> response1 = phi3ChatModel.generate(chatMemoryPhi3.messages());
        chatMemoryPhi3.add(response1.content());
        // Print Result
        ...
        UserMessage userMessage2 = userMessage(
                ...
                """);
        chatMemoryPhi3.add(userMessage2);
        Response<AiMessage> response2 = phi3ChatModel.generate(chatMemoryPhi3.messages());
        chatMemoryPhi3.add(response2.content());
        // Print Result
        AiBeans.printResult(userMessage2.text(), response2.content().text());
    }

    public static void chatMemoryWithMultipleUsers(ChatLanguageModel phi3ChatModel) {
        Assistant assistantPhi3 = AiServices.builder(Assistant.class)
                .chatLanguageModel(phi3ChatModel)
                .chatMemoryProvider(memoryId -> MessageWindowChatMemory.withMaxMessages(10))
                .build();

        String request1 = "UUID-1 >> Hello, my name is John Sam Doe";
        String response1 = assistantPhi3.chat("UUID-1", request1);
        AiBeans.printResult(request1, response1);

        String request2 = "UUID-2, >> Hello, my name is Jane Daisy Doe";
        String response2 = assistantPhi3.chat("UUID-2", request2);
        AiBeans.printResult(request2, response2);

        String request3 = "What is my name?";
        String response3 = assistantPhi3.chat("UUID-1", "UUID-1 >> "+request3);
        AiBeans.printResult("UUID-1 >> "+request3, response3);

        String response4 = assistantPhi3.chat("UUID-2", "UUID-2 >> "+request3);
    ...
    public static void main(String[] args) {
        ...
        ChatLanguageModel phi3ChatModel = AiBeans.getChatLanguageModelLlama(AiConstants.OLLAMA_PHI);
        AiBeans.printModelDetails(AiConstants.LLM_OLLAMA, AiConstants.OLLAMA_PHI);
        // Chat Memory Conversations
        chatMemoryConversations(phi3ChatModel);

        // Chat Memory with Multiple user
        chatMemoryWithMultipleUsers(phi3ChatModel);
    }
}
